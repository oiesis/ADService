...
int ipDataInit(const char *ipdb) {
    if (ipip.offset) {
        DebugMessageWithTime("offset failed");
        return -1;
    }
    FILE *file = fopen(ipdb, "rb");
    if(!file) {
        DebugMessageWithTime("failed to openFile");
        return -1;
    }
...
namespace adservice{
    namespace server{
        ...
        void IpManager::loadAreaData(const char* areafile){
            ...
            if(!file){
                DebugMessageWithTime("[ERROR] IpManager::loadAreaData areafile can not be opened! file:",areafile);
                return;
            }
            char buffer[256];
...
//                DebugMessageWithTime("country name:",iter->second.name);
            ...
            for(auto iter = provinceMap.begin();iter!=provinceMap.end();iter++){
                AreaDictAccessor acc;
//                DebugMessageWithTime("province name:",iter->second.name);
                areaDict.insert(acc,iter->second.name);
                acc->second = iter->second;
                acc.release();
            }
            for(auto iter = cityMap.begin();iter!=cityMap.end();iter++){
                AreaDictAccessor acc;
//                DebugMessageWithTime("city name:",iter->second.name);
                areaDict.insert(acc,iter->second.name);
                acc->second = iter->second;
                acc.release();
            }
        ...
        void IpManager::init(const char *ipdataFile, const char *areaCodeFile) {
            ...
            if(ipDataInit(ipdataFile)<0){
                DebugMessageWithTime("init ipdata from data file failed,file:",ipdataFile);
                return;
            }
