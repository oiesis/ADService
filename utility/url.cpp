...
namespace adservice{
    namespace utility{
        namespace url{
            ...
            bool processRequestLine(HttpRequest& request,const char* begin, const char* end)
            {
              bool succeed = false;
            ...
            bool parseHttpRequest(HttpRequest& request,const char* buffer,int size){
                static const char CRLF[]="\r\n";
                const char* p = buffer;
                const char* end = buffer+size;
                const char* pos = std::search(buffer,end,CRLF,CRLF+2);
                if(pos==end){
                    return false;
                }
                processRequestLine(request,p,pos);
                p=pos+2;
                bool meetEnd = false;
                while(p<end){
                    pos = std::search(p,end,CRLF,CRLF+2);
                    if(p==end){
                        return false;
                    }else if(p==pos){
//                        p+=2;
//                        if(std::search(p,end,CRLF,CRLF+2)!=p){
//                            return false;
//                        }
                        p+=2;
                        if(p<end){
                            std::string content = request.getHeader("Content-Length");
                            int contentLength = content.empty()?0:std::stoi(content);
                            if(contentLength>0)
                                request.setQuery(p,end);
                        }
                        break;
                    }else{
                        const char* colon = std::find(p,pos,':');
                        if(colon==pos){
                            return false;
                        }
                        request.addHeader(p,colon,pos);
                        p = pos+2;
                    }
                }
                return true;
            ...
            void urlEncode_f(const char* in,int len,std::string& output,char* buffer){
                ...
                while(i<len){
                    if(table.entry[in[i]]){
                        ...
                        buffer[j]='%';
                        buffer[j+1]=table.map[in[i]][0];
                        buffer[j+2]=table.map[in[i]][1];
                        j+=3;
                        i++;
                    }
                }
