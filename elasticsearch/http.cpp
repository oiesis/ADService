...
bool HTTP::error() {
    ...
    if(errno == EINVAL )
        printf("ElasticSearchException caught while reading socket - Invalid argument: _sfd = %i\n", _sockfd);
    ...
    printf("ElasticSearchException caught while reading socket - %s.\n",strerror(errno));
...
bool HTTP::request(const char* method, const char* endUrl, const char* data, rapidjson::Document& root, const char* content_type){
    Result result;
    request(method, endUrl, data, root, result, content_type);
    return (result == OK);
}
    ...
    try {
        if (output.size()) {
            if(!json::parseJson(output.c_str(),root))
                throw ElasticSearchException(__FILE__,__LINE__,"result json parse error");
        }
    }
    catch(ElasticSearchException& e){
        printf("parser() failed in Getter. ElasticSearchException caught: %s\n", e.what());
        result = ERROR;
        return statusCode;
    }
