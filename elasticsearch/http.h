...
#define EXCEPTION(...) throw ElasticSearchException(__FILE__, __LINE__, __VA_ARGS__)

class ElasticSearchException : std::exception {
    public:
        template<typename T>
        ElasticSearchException(const char* fil, int lin, T const& msg);
        virtual ~ElasticSearchException() throw() {}
...
template<typename T>
ElasticSearchException::ElasticSearchException(const char* fil, int lin, T const& msg) {
    _msg = msg;
...
class HTTP {
    public:
        ...
        bool request(const char* method, const char* endUrl, const char* data, rapidjson::Document& root, const char* content_type = _APPLICATION_JSON);

        /// Generic request that parses the result in Json::Object.
        unsigned int request(const char* method, const char* endUrl, const char* data, rapidjson::Document& root, Result& result, const char* content_type = _APPLICATION_JSON);
